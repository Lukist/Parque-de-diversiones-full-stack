...
public class HorarioJpaController implements Serializable {
    ...
    public void create(Horario horario) {
        ...
        try {
            ...
            Atraccion atra = horario.getAtra();
            if (atra != null) {
                atra = em.getReference(atra.getClass(), atra.getIdAtraccion());
                horario.setAtra(atra);
            }
            em.persist(horario);
            if (atra != null) {
                atra.getListaHor().add(horario);
                atra = em.merge(atra);
            }
    ...
    public void edit(Horario horario) throws NonexistentEntityException, Exception {
        ...
        try {
            ...
            Atraccion atraOld = persistentHorario.getAtra();
            Atraccion atraNew = horario.getAtra();
            if (atraNew != null) {
                atraNew = em.getReference(atraNew.getClass(), atraNew.getIdAtraccion());
                horario.setAtra(atraNew);
            }
            horario = em.merge(horario);
            if (atraOld != null && !atraOld.equals(atraNew)) {
                atraOld.getListaHor().remove(horario);
                atraOld = em.merge(atraOld);
            }
            if (atraNew != null && !atraNew.equals(atraOld)) {
                atraNew.getListaHor().add(horario);
                atraNew = em.merge(atraNew);
            }
            em.getTransaction().commit();
        } catch (Exception ex) {
    ...
    public void destroy(long id) throws NonexistentEntityException {
        ...
        try {
            ...
            Atraccion atra = horario.getAtra();
            if (atra != null) {
                atra.getListaHor().remove(horario);
                atra = em.merge(atra);
            }
            em.remove(horario);
            em.getTransaction().commit();
